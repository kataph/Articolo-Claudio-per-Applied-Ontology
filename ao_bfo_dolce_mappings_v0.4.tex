...
\begin{abstract}
\noindent 
The development of an ontology-based ecosystem suitable for interoperability and for the reliable exchange of data is a longstanding goal of the applied ontology community. The construction of such a system relies on alignments across a variety of ontologies: foundational, reference and domain. While the techniques for the alignment of reference and domain ontologies have been continuously studied for more than 15 years; work on the techniques for the alignment of foundational ontologies and across foundational and reference ontologies have been scattered or hardly addressed. %Still today, the topic remains largely unexplored. 
This paper aims to bring attention to the issue in three ways. It motivates the research, presents an approach to develop formal alignments of foundational ontologies, and implements its applicability to study the relationship among two of them, namely, {\bfo} and {\dolce}. The paper points also to variants of and alternatives to this approach by briefly comparing advantages and difficulties.
\end{abstract}
%==========================================
...
\bflist
\item[\dolcedf{def_PPd}] $\PPd(x,y) \sdef \Pd(x,y) \land \neg\Pd(y,x)$ 
\mbox{} \hfill{} (\emph{Proper Part})

\item[\dolcedf{def_Od}] $\Od(x,y) \sdef \exists z (\Pd(z,x) \land \Pd(z,y))$    
\mbox{} \hfill{} (\emph{Overlap})

\item[\dolcedf{def_ATd}] $\ATd(x) \sdef (\PDdcat(x) \lor \ABdcat(x)) \land \neg\exists y (\PPd(y,x))$
\mbox{} \hfill{} (\emph{Atom})
  
\item[\dolcedf{def_SUMd}] $\SUMd(s,x,y) \sdef \forall z (\Od(z,s) \ifif (\Od(z,x) \lor \Od(z,y)))$  
\mbox{} \hfill{} (\emph{Mereological Sum})

\item[\dolcedf{def_TPPd}] $\TPPd(x,y,t) \sdef \TPd(x,y,t) \land \neg\TPd(y,x,t)$
\mbox{} \hfill{} (\emph{Temporary Proper Part})

\item[\dolcedf{def_TOd}] $\TOd(x,y,t) \sdef \exists z (\TPd(z,x,t) \land \TPd(z,y,t))$  
\mbox{} \hfill{} (\emph{Temporary Overlap})

\item[\dolcedf{dfPREd}] $\PREd(x,t) \sdef \exists u(\TLCd(x,u) \land \Pd(t,u))$  
\mbox{} \hfill{} (\emph{Being Present in Time})

\item[\dolcedf{dfSDd}] $\SDd(x,y) \sdef \exists t (\PREd(x,t)) \land \forall t (\PREd(x,t) \to \EXDd(x,y,t))$ 
\mbox{} \hfill{} (\emph{Specific Dependence})

\item[\dolcedf{dfSPREd}] $\SPREd(x,s,t) \sdef \exists r(\SLCd(x,r,t) \land \Pd(s,r))$  
\mbox{} \hfill{} (\emph{Being Present in Space})
\eflist
...
For the sake of brevity, some of the relations are skipped by exploiting taxonomical axioms. That is, we add the axioms of the {$\thdolce$} (or  {$\thbfo$}) taxonomy, %to simplify our work, 
...
These syntactic definitions are given below %---where, in {\dolce}, $\ATd(x) \sdef \forall y(\Pd(y,x) \to x=y)$---
together with a short informal description. (Once a definition has been introduced, it may be used only in definitions given later to avoid circularity.) A deeper analysis about these definitions and their impact on the preservation of the axioms of {\bfo} is in Sect.~\ref{sect_analysis_d2b}. 
%
\bflist
...
Time intervals $\bfoexist$-exist at every subinterval of themselves while the other abstracts entities $\bfoexist$-exist at every time. This extension matches the fact that, in {\bfo}, $\bfoexist$ is defined on both temporal regions and spatial regions (which seem very close to {\dolce} time intervals and space regions, respectively, see \refdbdf{d2b_treg} and \refdbdf{d2b_sreg}).
...
For instance, in {\dolce} it is possible to have three temporally co-localized perdurants $p_1$, $p_2$, and $p_{12}$ all with atomic temporal locations and such that  $p_{12}$ is the mereological sum of $p_1$ and $p_2$. %, i.e., $\SUMd(p_{12}, p_1,p_2)$. 
...
\begin{tabular}{|>{\centering\arraybackslash}b{.03\textwidth}|>{\raggedright\arraybackslash}b{.08\textwidth}|p{.90\textwidth}|}
\hline
$\oplus$ & \dolceax{PdArg} & $\Pd(x,y) \to (\ABdcat(x) \land \ABdcat(y)) \lor (\PDdcat(x) \land \PDdcat(y))$  \\ \hline
$\oplus$ & \dolceax{PdArgT} & $\Pd(x,y) \to (\Tdcat(x) \ifif \Tdcat(y))$  \\ \hline
$\oplus$ & \dolceax{PdArgS} & $\Pd(x,y) \to (\Sdcat(x) \ifif \Sdcat(y))$  \\ \hline 
$\oplus$ & \dolceax{PdRefl} & $(\ABdcat(x) \lor \PDdcat(x)) \to \Pd(x,x)$ \\ \hline 
$\oplus$ & \dolceax{PdAntiSym} & $\Pd(x,y) \land \Pd(y,x) \to x=y$  \\ \hline
$\oplus$ & \dolceax{PdTrans} & $\Pd(x,y) \land \Pd(y,z) \to \Pd(x,z)$  \\ \hline
$\odot$ & \dolceax{PdSupp} & $(\ABdcat(x) \lor \PDdcat(x)) \land \neg\Pd(x,y) \to \exists z (\Pd(z,x) \land \neg \Od(z,y))$ \\ \hline
$\ominus$ & \dolceax{DQTd_args} & $\DQTd(x,y) \to (\TQdcat(x) \land \PDdcat(y)) \lor (\PQdcat(x) \land \PEDdcat(y)) \lor \newline \mbox{} \hspace{1.41cm} (\AQdcat(x) \land \NPEDdcat(y))$  \\ \hline
$\odot$ & \dolceax{DQTdUnicity} & $\DQTd(x,y) \land \DQTd(x,z) \to y=z$  \\ \hline
$\oplus$ & \dolceax{DQTdExistence} & $\Qdcat(x) \to \exists y (\DQTd(x,y))$  \\ \hline
$\odot$ & \dolceax{DQTd_PD} & $\PDdcat(x) \ifif \exists y (\DQTd(y,x) \land \TLdcat(y))$  \\ \hline
$\odot$ & \dolceax{DQTd_PED} & $\PEDdcat(x) \ifif \exists y (\DQTd(y,x) \land \SLdcat(y))$  \\ \hline 
$\otimes$ & \dolceax{QLd_args} & $\QLd(x,y) \to \TRdcat(x) \land \TQdcat(y)$  \\ \hline
$\otimes$ & \dolceax{TQdcat_to_QLd} & $\TQdcat(x) \to \exists y (\QLd(y,x))$  \\ \hline
$\otimes$ & \dolceax{Tdcat_to_TLdcat} & $\QLd(x,y) \land \Tdcat(x) \to \TLdcat(y)$  \\ \hline
$\otimes$ & \dolceax{TLdcat_to_Tdcat} & $\TLdcat(x) \to \exists y(\Tdcat(y) \land \QLd(y,x))$ \\ \hline
$\otimes$ & \dolceax{Tdcat_unicity} & $\QLd(x,y) \land \QLd(z,y) \land \Tdcat(x) \land \Tdcat(z) \to x=z$ \\ \hline
$\oplus$ & \dolceax{TLCd_args} & $\TLCd(x,t) \to (\EDdcat(x) \lor \PDdcat(x) \lor \Qdcat(x)) \land \Tdcat(t)$ \\ \hline
$\odot$ & \dolceax{d_partictime} & $(\EDdcat(x) \lor \PDdcat(x) \lor \Qdcat(x)) \to \exists t(\TLCd(x,t))$ \\ \hline
$\oplus$ & \dolceax{TLCd_unicity} & $\TLCd(x,t) \land \TLCd(x,u) \to t=u$ \\ \hline
$\oplus$ & \dolceax{TLCd_additiv} & $\Pd(x,y) \land \TLCd(x,t) \land \TLCd(y,u) \to \Pd(t,u)$ \\ \hline
$\oplus$ & \dolceax{TLCd_Q} & $\DQTd(x,y) \land \TLCd(x,t) \land \TLCd(y,u) \to \Pd(t,u)$  \\ \hline
$\otimes$ & \dolceax{TLCd_PD} & $\PDdcat(x) \to (\TLCd(x,t) \ifif \exists y(\DQTd(y,x) \land \QLd(t,y) \land \Tdcat(t)))$ \\ \hline
$\otimes$ & \dolceax{TLCd_SL} & $\SLdcat(x) \land \DQTd(x,y) \land \TLCd(x,t) \land \TLCd(y,u) \to t=u$ \\ \hline
$\otimes$ & \dolceax{TQLd_args} & $\TQLd(x,y,t) \to ((\PRdcat(x) \land \PQdcat(y)) \lor (\ARdcat(x) \land \AQdcat(y))) \land \Tdcat(t)$ \\ \hline% label(temporary_quale_argument_restrictions_Ad58)
$\otimes$ & \dolceax{TQLd_to_PREd} & $\TQLd(x,y,t) \to \PREd(y,t)$ \\ \hline% label(temporary_quale_present_Ad65)
$\otimes$ & \dolceax{TQLd_existence} & $(\PQdcat(x) \lor \AQdcat(x)) \land \PREd(x,t) \to \exists y (\TQLd(y,x,t))$ \\ \hline%label(temporary_quale_existential_Ad62)
$\otimes$ & \dolceax{TQLd_dissect} & $\TQLd(x,y,t) \land \Pd(u,t) \to \exists z(\TQLd(z,y,u) \land \Pd(z,x))$  \\ \hline
$\otimes$ & \dolceax{Sdcat_to_SLdcat} & $\TQLd(x,y,t) \land \Sdcat(x) \to \SLdcat(y)$ \\ \hline
$\otimes$ & \dolceax{SLdcat_to_Sdcat} & $\SLdcat(x) \land \PREd(x,t) \to \exists y(\Sdcat(y) \land \TQLd(y,x,t))$ \\ \hline
$\otimes$ & \dolceax{Sldcat_unicity} & $\TQLd(x,y,t) \land \TQLd(z,y,t) \land \Sdcat(x) \land \Sdcat(z) \to x=z$ \\ \hline
$\oplus$ & \dolceax{TPd_args} & $\TPd(x,y,t) \to \EDdcat(x) \land \EDdcat(y) \land \Tdcat(t)$ \\ \hline% 
$\odot$ & \dolceax{TPd_exist} & $\TPd(x,y,t) \to \PREd(x,t) \land \PREd(y,t)$ \\ \hline% label(temporary_parthood_Ad17)
$\oplus$ & \dolceax{TPd_temp_dissectivity} & $\TPd(x,y,t) \land \Pd(u,t) \to \TPd(x,y,u)$ \\ \hline% label(temporary_parthood_Ad18)
$\odot$ & \dolceax{TPd_temp_addictivity} & $\TPd(x,y,t) \land \TPd(x,y,u) \land \SUMd(s,t,u) \to \TPd(x,y,s)$ \\ \hline
$\odot$ & \dolceax{TPd_reflex} & $\EDdcat(x) \land \PREd(x,t) \to \TPd(x,x,t)$ \\ \hline% label(temporary_parthood_Ad16)
$\oplus$ & \dolceax{TPd_trans} & $\TPd(x,y,t) \land \TPd(y,z,t) \to \TPd(x,z,t)$ \\ \hline% label(temporary_parthood_ground_axiom_Ad13)
$\odot$ & \dolceax{TPd_ssp} & $\EDdcat(x) \land \EDdcat(y) \land \PREd(x,t) \land \PREd(y,t) \land \neg \TPd(x,y,t) \to \newline \mbox{} \hfill{} \exists z (\TPd(z,x,t) \land \neg \TOd(z,y,t))$ \\ \hline
$\oplus$ & \dolceax{TPd_PED} & $\PEDdcat(x) \ifif \exists yt(\TPd(y,x,t)) \land \forall yt(\TPd(y,x,t) \to \PEDdcat(y))$ \\ \hline 
$\odot$ & \dolceax{TPd_NPED} & $\NPEDdcat(x) \ifif  \exists yt(\TPd(y,x,t)) \land \forall yt(\TPd(y,x,t) \to \NPEDdcat(y))$ \\ \hline
$\otimes$ & \dolceax{TPd_AS} & $\ASdcat(x) \to \newline \mbox{} \hfill{} \exists yzut(\TPd(y,x,t) \land \PEDdcat(y) \land \TPd(z,x,u) \land \NPEDdcat(z))$ \\ \hline 
$\otimes$ & \dolceax{TPd_M} & $\Mdcat(x) \land \TPd(y,x,t) \land \PREd(x,u) \to \TPd(y,x,u)$  \\ \hline
$\otimes$ & \dolceax{Kd_args} & $\Kd(x,y,t) \to \Tdcat(t) \land ((\PEDdcat(x){\land} \PEDdcat(y)) \lor \newline \mbox{} \hfill{}  (\NPEDdcat(x){\land}\NPEDdcat(y)) \lor  (\PDdcat(x){\land}\PDdcat(y)))$ \\ \hline
...
$\otimes$ & \dolceax{Kd_to_PREd} & $\Kd(x,y,t) \to \PREd(x,t) \land \PREd(y,t)$ \\ \hline% label(constitution_Ad26)
$\otimes$ & \dolceax{Kd_dissect} & $\Kd(x,y,t) \land \Pd(u,t) \to \Kd(x,y,u)$ \\ \hline% label(constitution_Ad27_weakened)
$\otimes$ & \dolceax{Kd_temp_addictivity} & $\Kd(x,y,t) \land \Kd(x,y,u) \land \SUMd(s,t,u) \to \Kd(x,y,s)$ \\ \hline
$\otimes$ & \dolceax{Kd_existdep} & $\Kd(x,y,u) \land \PREd(y,t) \to \exists zv(\Pd(v,t) \land \Kd(z,y,v))$ \\ \hline
$\otimes$ & \dolceax{Kd_asym} & $\Kd(x,y,t) \to \neg\Kd(y,x,t)$ \\ \hline% label(constitution__ground_axiom_Ad24)
$\otimes$ & \dolceax{Kd_trans} & $\Kd(x,y,t) \land \Kd(y,z,t) \to \Kd(x,z,t)$ \\ \hline% label(constitution__ground_axiom_Ad25)
$\otimes$ & \dolceax{Kd_granularity} & $\Kd(x,y,t) \land \TPPd(z,y,t) \to \exists w(\TPPd(w,x,t) \land \Kd(w,z,t))$ \\ \hline% label(constitution_Ad29) 
$\otimes$ & \dolceax{Kd_unicity} & $\Kd(x,y,t) \land \Kd(z,y,t) \to (x=z \lor \Kd(x,z,t) \lor \Kd(z,x,t))$  \\ \hline
$\oplus$ & \dolceax{PCd_args} & $\PCd(x,y,t) \to \EDdcat(x) \land \PDdcat(y) \land \Tdcat(t)$ \\ \hline% label(participation_argument_restrictions_Ad33)
$\odot$ & \dolceax{PCd_present} & $\PCd(x,y,t) \to \PREd(x,t) \land \PREd(y,t)$ \\ \hline% label(participation_present_Ad36)
$\oplus$ & \dolceax{PCd_time_dissect} & $\PCd(x,y,t) \land \Pd(u,t) \to \PCd(x,y,u)$ \\ \hline% label(participation_present_Ad37)
$\odot$ & \dolceax{PCd_temp_addictivity} & $\PCd(x,y,t) \land \PCd(x,y,u) \land \SUMd(s,t,u) \to \PCd(x,y,s)$ \\ \hline
$\odot$ & \dolceax{PCd_existence_participant} & $\PDdcat(x) \land \PREd(x,t) \to \exists yu(\Pd(u,t) \land \PCd(y,x,u))$ \\ \hline% label(participation_existential_Ad34)
$\odot$ & \dolceax{PCd_existence_perdurant} & $\EDdcat(x) \land \PREd(x,t) \to \exists yu (\Pd(u,t) \land \PCd(x,y,u))$ \\ \hline% label(participation_existential_Ad35_corrected)
$\odot$ & \dolceax{PDd_pd_addict} & $\PCd(x,y,t) \land \Pd(y,z) \to \PCd(x,z,t)$ \\ \hline
$\odot$ & \dolceax{EXDd2PREd} & $\EXDd(x,y,t) \to \PREd(x,t) \land \PREd(y,t)$ \\ \hline
$\odot$ & \dolceax{EXDd_time_dissect} & $\EXDd(x,y,t) \land \Pd(u,t) \to \EXDd(x,y,u)$ \\ \hline
$\odot$ & \dolceax{EXDd_temp_addictivity} & $\EXDd(x,y,t) \land \EXDd(x,y,u) \land \SUMd(s,t,u) \to \EXDd(x,y,s)$  \\ \hline
$\odot$ & \dolceax{EXDd_existdep} & $\EXDd(x,y,u) \land \PREd(x,t) \to \exists zv(\Pd(v,t) \land \EXDd(x,z,v))$ \\ \hline
$\otimes$ & \dolceax{EXDd_to_Kd} & $\Kd(x,y,t) \to \EXDd(y,x,t)$   \\ \hline
$\oplus$ & \dolceax{EXDd_to_PCd} & $\PCd(x,y,t) \to \EXDd(x,y,t) \land \EXDd(y,x,t)$  \\ \hline%\hfill{\color{red} [added]}
$\odot$ & \dolceax{DQTd_to_SDd} & $\DQTd(x,y) \to \SDd(x,y)$  \\ \hline% label(direct_quality_Ad43)
$\odot$ & \dolceax{EXDd_NPED} & $\NPEDdcat(x) \land \PREd(x,t) \to \exists yu(\PEDdcat(y) \land \Pd(u,t) \land \EXDd(x,y,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_MOB} & $\MOBdcat(x) \to \exists y(\APOdcat(y) \land \SDd(x,y))$  \\ \hline
$\otimes$ & \dolceax{EXDd_NASO} & $\NASOdcat(x) \land \PREd(x,t) \to \exists yu(\SCdcat(y) \land \Pd(u,t) \land \EXDd(x,y,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_SC} & $\SCdcat(x) \land \PREd(x,t) \to \exists yu(\SAGdcat(y) \land \Pd(u,t) \land \Kd(y,x,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_SAG} & $\SAGdcat(x) \land \PREd(x,t) \to \exists yu(\APOdcat(y) \land \Pd(u,t) \land \EXDd(x,y,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_F} & $\Fdcat(x) \land \PREd(x,t) \to \exists yu(\NAPOdcat(y) \land \Pd(u,t) \land \EXDd(x,y,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_APO} & $\APOdcat(x) \land \PREd(x,t) \to \exists yu(\NAPOdcat(y) \land \Pd(u,t) \land \Kd(y,x,u))$ \\ \hline
$\otimes$ & \dolceax{EXDd_NAPO} & $\NAPOdcat(x) \land \PREd(x,t) \to \exists yu(\Mdcat(y) \land \Pd(u,t) \land \Kd(y,x,u))$ \\ \hline
$\ominus$ & \dolceax{SLCd_args} & $\SLCd(x,s,t) \to ((\EDdcat(x) \land \neg \ASdcat(x)) \lor \PQdcat(x) \lor \PDdcat(x)) \land \Sdcat(s)$ \\ \hline
$\odot$ & \dolceax{SLCd_to_PREd} & $\SLCd(x,s,t) \to \PREd(x,t)$ \\ \hline
$\oplus$ & \dolceax{SLCd_unicity} & $\SLCd(x,s,t) \land \SLCd(x,r,t) \to s=r$  \\ \hline
$\oplus$ & \dolceax{SLCd_dissectivity} & $\SLCd(x,s,u) \land \SLCd(x,r,t) \land \Pd(u,t) \to \Pd(s,r)$  \\ \hline
$\odot$ & \dolceax{SLCd_existdep} & $\SLCd(x,s,u) \land \PREd(x,t) \to \exists r(\SLCd(x,r,t))$  \\ \hline
$\otimes$ & \dolceax{SLCd_PEDs} & $\PEDdcat(x) \to \newline \mbox{} \hfill{}  (\SLCd(x,s,t) \ifif \exists y(\DQTd(y,x) \land \SLdcat(y) \land \TQLd(s,y,t)))$  \\ \hline
$\otimes$ & \dolceax{SLCd_PQ} & $\PQdcat(x) \to (\SLCd(x,s,t) \ifif \exists y(\DQTd(x,y) \land \SLCd(y,s,t)))$  \\ \hline
$\oplus$ & \dolceax{dolce_NPED_sloc} & $\NPEDdcat(x) \land \SLCd(x,s,t) \to \newline \mbox{} \hfill{}  \exists y(\PEDdcat(y) \land \EXDd(x,y,t) \land \SLCd(y,s,t))$  \\ \hline
$\odot$ & \dolceax{SLCd_PDs1} & $\PCd(x,y,t) \land \SLCd(x,s,t) \to \exists r(\SLCd(y,r,t))$  \\ \hline
$\oplus$ & \dolceax{SLCd_PDs2} & $\PCd(x,y,t) \land \SLCd(x,r,t) \land \SLCd(y,s,t) \to \Pd(r,s)$  \\ \hline
$\odot$ & \dolceax{TPd_up_slc} & $\TPd(x,y,t) \land \SLCd(x,s,t) \to \exists r (\SLCd(y,r,t))$  \\ \hline
$\oplus$ & \dolceax{TPd_down_slc} & $\TPd(x,y,t) \land \SLCd(x,r,t) \land \SLCd(y,s,t)  \to \Pd(r,s)$ \\ \hline
$\odot$ & \dolceax{Pd_up_slc} & $\Pd(x,y) \land \SLCd(x,s,t) \to \exists r(\SLCd(y,r,t))$  \\ \hline
$\oplus$ & \dolceax{Pd_down_slc} & $\Pd(x,y) \land \SLCd(x,r,t) \land \SLCd(y,s,t) \to \Pd(r,s)$  \\ \hline
$\otimes$ & \dolceax{Kd_colocalization} & $\Kd(x,y,t) \to \forall s(\SLCd(x,s,t) \ifif \SLCd(y,s,t))$  \\ \hline
\end{tabular}
\end{scriptsize}
\end{minipage}%
%\end{footnotesize}
